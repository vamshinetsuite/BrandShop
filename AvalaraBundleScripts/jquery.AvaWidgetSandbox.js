...
(function ($) {
    jQuery.fn.AvaWidgetSandbox = function (options) {
        //default settings:
        var defaults = {
            InheritCss: false,
        ...
        return this.each(function () {
            ...
            AuthenticateSandbox(function (data) {
                if (data.IsAuthenticated) {
                    settings.AccessToken = data.AccessToken;
                    settings.AvalaraIdentity = data.AvalaraIdentity;
                    htmlToSet = '<iframe src="https://sandbox.provisioningforavatax.com/onboarding/defaultIndex" frameborder="0" scrolling="yes" style="width:100%;height:100%" id="myFrame" onload="javascript:sendMessageSandbox(\'' + encodeURI(JSON.stringify(settings)) + '\',\'' + publicKey + '\');">';
                    elem.html(htmlToSet);
                } else {
...
function sendMessageSandbox(options, publicKey) {
    var receiver = document.getElementById('myFrame').contentWindow;
    options = eval("(" + decodeURI(options) + ")");
    receiver.postMessage(options, "https://sandbox.provisioningforavatax.com/onboarding/defaultIndex");
}
function GetCssURLsSandbox() {
    var cssLinks = [];
...
function AuthenticateSandbox(callbackFunction) {

    var AuthenticateAjax = NS.jQuery.ajax({
        type: "POST",
        url: "https://sandbox.provisioningforavatax.com/AvaAuthenticator/Authentcate",
        ...
        success: function (data) {
        	InvokeCallbackAuthenticateSandbox(callbackFunction, data);
        },
...
function InvokeCallbackAuthenticateSandbox(callback, data) {
...
function SetDefaultValueSandbox(dataToBeVerified) {
